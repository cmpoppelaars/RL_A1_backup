...
class DQN_TN:
    ...
    def update(self, states, actions, rewards, n_states, done, old_dones):
         # Set target
        yj = rewards.copy()

        # Update only the non-initial states and non-terminated states 
        next_q_values = self.TargetNetwork(n_states[~old_dones * ~done]).numpy()
        yj[~old_dones * ~done] += self.gamma * np.max(next_q_values, axis=1)

        # Update the Q-values of the selected actions in batch
        target_q_values = self.MainNetwork(states, training=False).numpy()
        target_q_values[range(len(actions)), actions] = yj
        ...
        # Perform training step for batch of states -> select the ratio of data-to-update
        self.MainNetwork.fit(states[~old_dones][:slice_up_to, :], target_q_values[~old_dones][:slice_up_to, :], verbose=0)
    ...
    def train_agent(self):
        ...
        while step < self.n_steps:
            ...
            # NOTE: training the main network
            self.update(states, actions,rewards, n_states, done, old_dones)
            ...
            # Update state and dones for each environment
            states = n_states  
            old_dones = done
