...
class DQN_ER:
    ...
    def experience_replay(self, batch_size):
        ...
        # Update only the non-terminated steps and states that are not the initial state
        next_q_values = self.MainNetwork(
            sampled_n_states[~sampled_old_dones * ~sampled_dones]
        ).numpy()

        yj[~sampled_old_dones * ~sampled_dones] += self.gamma * np.max(
        ...
        slice_up_to = int(
            sampled_states[~sampled_old_dones].shape[0] * self.update_to_data_ratio
        )
        ...
        while eps_done < self.n_eval_episodes:
            ...
            if n_done > 0:
                ...
                if len(returns) + n_done > self.n_eval_episodes:
                    ...
                    returns.extend(
                        ep_rewards[done][: self.n_eval_episodes - len(returns)]
                    )
